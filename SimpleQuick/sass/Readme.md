...
folder are mapped to the application's root namespace, 'SimpleQuick'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "SimpleQuick"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'SimpleQuick' in the same way
as `"SimpleQuick/sass/src"`.
